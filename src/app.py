...
def listar_cursos(): # Creamos la funcion listar
    try:
        ...
        cursos=[]
        for fila in datos:
            curso = {'codigo':fila[0], 'creditos':fila[1], 'nombre':fila[2]}#Creamos diccionario para almacenar los datos en JSON
...
def registrar_curso():
    try:
        #print(request.json)
        #return jsonify({'data': request.json["nombre"]})
        cursor=conexion.connection.cursor()
...
if __name__=='__main__':#Se hace la comprobación
    ...
    app.run(debug=True)# DSi es así ejecutamos la app mediante el método run en modo dev